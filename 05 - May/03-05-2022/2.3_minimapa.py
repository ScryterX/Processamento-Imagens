...
img_erodida=cv2.erode(img_bin, ee, iterations=1)
cv2.imshow("bin", img_erodida)
contornos, h = cv2.findContours(img_erodida, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
area = cv2.contourArea(contornos[0])
img = cv2.drawContours(img_original, contornos, -1, (190,0,190),2)
...
cv2.putText(img, f"área da montanha em pixels:{area}",(20, 20), FONT_HERSHEY_SIMPLEX, 0.4, (150, 30, 150),1)
cv2.imshow("contornada", img)
